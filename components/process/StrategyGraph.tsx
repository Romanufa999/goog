...
const Node: React.FC<{ text: string; delay: number; position: string }> = ({ text, delay, position }) => (
    <div 
        className={`absolute ${position} flex items-center justify-center w-24 h-24 rounded-full bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border border-black/10 dark:border-white/10 text-center text-sm font-semibold p-2 shadow-lg opacity-0 animate-node-appear`}
